...
SailPlan is a sailing app. This vocabulary is assumed everywhere in the code and
can't be inferred from types alone — read it before working on the Plan, Sail,
Polar, or Suggestion features. Each term links to where it lives in code.
...
[plan store](../features/plan/store/planStore.ts)). TWA is **computed** from TWD
and the boat's bearing — see [`getTwa`](../features/coordinate/util/bearing.ts).

## Direction & geometry

| Term        | Meaning                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------ |
| **Bearing** | Compass direction (0–360°) from one point to another, i.e. the heading to sail a leg. Computed by [`coordsToBearing`](../features/coordinate/util/bearing.ts) (great-circle formula). |
...
[`getTwa`](../features/coordinate/util/bearing.ts) returns both the **angle** and
the **tack**:
...
sail suggestion ([`getWindZone`](../features/sailSuggestion/model/windZone.ts)):
...
| **Limit score** | A bell-curve score of how well the requested TWA sits inside a sail's limits — 1.0 at the centre of the band, tapering to negative outside. Used when polar data is sparse. See [`limitScoring`](../features/sailSuggestion/util/limitScoring.ts). |
...
helpers are in
[`features/coordinate/util/mappers.ts`](../features/coordinate/util/mappers.ts).
...
[architecture.md](architecture.md#provider--startup-chain)). Switching profiles
...
See [`features/plan`](../features/plan/README.md) and
[`features/sailSuggestion`](../features/sailSuggestion/README.md) for the
mechanics.
