...
| **capture**      | NMEA capture: plotter setup (manual host/port), and **raw-log recording** — connect over boat Wi-Fi, stream the verbatim NMEA to app document storage, and hold the process open in a `connectedDevice` foreground service. Review of the sailed legs — a session screen listing what each leg yields, and one route per leg where edits save as they are made (`reviewedAt` records that a leg was *changed*, not that it was opened) — and **promotion** — reviewed spans intersected with the steadiness mask, binned to the clustered grid and offered as polar points against the sailor's existing table before anything is written. Also the schema mirror for sessions, samples, connection events, sail stamps, sailed legs and spans. | `store/captureRecordingStore`, `util/captureRecorder`, `util/rawLog`, `util/captureForegroundService`, `CaptureRecordingBar` (the capture layer), `CaptureRecordingControl`, `PlotterConnectionSection`, `util/steadyState`, `util/legReview`, `util/traceGeometry`, `hooks/useCaptureReview`, `util/promotion`, `util/promotionReview`, `CaptureSessionReview`, `SailedLegReview`, `CapturePromotion` |
...
| **course**       | CRUD for courses, course marks (ordered), and course groups.                                       | `api/*Course`, `CourseForm`, `CourseMarks`, models `course`/`courseMark`/`courseGroup` |
...
| **sailPolar**    | Sail polar data: CRUD, CSV import as removable **batches** (fingerprinted against duplicate re-import), polar/scatter charts, **grid-aware bilinear interpolation** (IDW fallback) with confidence, the **source-aware blend** that keeps captured points off the imported grid, and the single insert path promotion writes captured points through. | `util/interpolation` (`estimateSailSpeed`), `util/sourceAwareInterpolation` (`estimateSourceAwareSailSpeed`), `util/importFingerprint`, `api/capturePolarPoints`, `PolarPlotChart`, `ScatterChart`, `SailPolarImportDialog`, `PolarImportBatches` · [README](../features/sailPolar/README.md) |
