...
| **sailPolar**    | Sail polar data: CRUD, CSV import as removable **batches** (fingerprinted against duplicate re-import), polar/scatter charts, **grid-aware bilinear interpolation** (IDW fallback) with confidence, and the **source-aware blend** that keeps captured points off the imported grid. | `util/interpolation` (`estimateSailSpeed`), `util/sourceAwareInterpolation` (`estimateSourceAwareSailSpeed`), `util/importFingerprint`, `PolarPlotChart`, `ScatterChart`, `SailPolarImportDialog`, `PolarImportBatches` · [README](../features/sailPolar/README.md) |
