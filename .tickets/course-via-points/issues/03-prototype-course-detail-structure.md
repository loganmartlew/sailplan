...
- ~~The sailor selects a named **Leg Segment** before adding. Selection is visible in both the detail list and map, so overlapping geometry never requires guessing a line.~~ **Superseded by [07](07-prototype-integrated-course-editing-flow.md)** — a persistent selection mode was rejected as a desktop idiom. There is no selected Segment; the Route thread's per-gap **⊕ Insert** rows and the map's nearest-Segment inference scope an insertion for exactly as long as the add lasts. The concern this bullet was answering — overlapping geometry requiring a guess — is now handled by naming the inferred target in words before the confirm.
- **Add Via Point** opens one chooser: **Map pin** or **Existing Mark**. A Map pin assigns a stable default name selected for replacement, and is placed one at a time. An Existing Mark becomes a Mark-backed Via Point in the targeted gap.
...
- The expanded Course Map supports adding through the same chooser (its target inferred from the tap position, per [07](07-prototype-integrated-course-editing-flow.md)) and moving course-local Via Point pins. It shows route shape only; it makes no navigability/safety claim and Via Points have no rounding direction.
