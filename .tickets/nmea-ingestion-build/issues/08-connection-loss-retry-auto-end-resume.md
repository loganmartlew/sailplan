...
- [ ] **One policy, measured from the last valid NMEA anchor data**, so a
      connected-but-silent socket is covered as well as a closed one
- [ ] Live values grey out and a **Retrying** bar with the gap age appears
      immediately on loss
- [ ] The notification changes to **Connection lost — retrying** immediately
- [ ] A distinct vibration ~5 s after the loss, one reminder at 60 s, then **no
      more**; one short confirmation vibration on recovery; one final vibration
      at auto-end
- [ ] Retry immediately, then at 1 s, 2 s, 5 s and 10 s, then every 15 s
- [ ] Recovery continues **the same capture session**; the outage is preserved as
      a `connectionEvent` gap. **No manufactured empty sample rows, ever**
- [ ] Auto-end at a **named constant defaulting to 30 minutes** — the spec call
...
- [ ] Auto-end sets the end time to the **last valid sample** so trailing silence
      contributes nothing, sets `status: 'autoEnded'`, stops the service, and
      posts a `Recording ended — plotter data lost` notification
- [ ] **Resume recording** appears only in that notification and on the original
      ...
      **same** session, keeps the outage as a gap, and advances the end time when
      capture later stops
- [ ] The resume offer disappears on dismissal (`resumeDismissedAt`), on another
...
- [ ] A recording stopped **deliberately** can never resume, and recovery never
      starts a new session automatically
- [ ] Resumability is **derived**, not a fourth status: `autoEnded` ∧ no
