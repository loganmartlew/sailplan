...
- [x] `sailPolar.sourceKind` exists, `notNull`, values `manual` | `import` | `capture`
- [x] **No ORM-level default** — the DDL default exists purely to backfill, so a
      writer that forgets the column fails loudly
- [x] The generated migration is a plain `ALTER TABLE … ADD COLUMN` and contains
      nothing else: no foreign key, no second column, no index
- [x] Every existing row backfills to `manual` (no `legacy` value — sole user,
      nothing imported, so the unknown is not unknown)
- [x] CSV import writes `import`; manual add writes `manual`; each goes through a
      single insert path that sets it explicitly
- [x] The generated SQL is read and understood before it is trusted
- [x] Applied against a **populated** database — a real device DB with existing
      polar rows, not an empty one — and the app opens
- [x] Full suite green; existing polar tests keep their contract
