...
| **Reviewed** (of a sailed leg) | That the sailor has seen this leg and their edits to it are stored. Reviewing is not a separate confirming act — moving to another leg is what records it, so review can be paused and resumed without losing work. It is a statement about *attention*, not about quality: a reviewed leg may still be marked **not used**. Distinct from **promotion**, which is one deliberate session-level decision taken later against the boat's existing polar table. An unreviewed leg carries only draft spans, so it contributes nothing. |
