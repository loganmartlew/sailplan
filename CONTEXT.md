...
| **Sail-attribution span** | An interval within a sailed leg assigned to one sail — or to none, which review calls **no sail**. The app may infer draft spans from sail stamps and surrounding conditions, but a draft is the app's hypothesis, not the sailor's claim: only spans belonging to a **reviewed** leg can contribute to polar promotion. Spoken of as a **block** in review, where a leg reads as a row of them. |
| **Reviewed** (of a sailed leg) | That the sailor has **changed** this leg — its spans, its name, or whether it is used — and the change is stored. Set by an edit, never by a visit: a leg merely opened and left has no hand work to lose, and calling that reviewed would both admit the app's own draft attribution to the polar and make the re-detect warning cry wolf. There is no separate confirming act; edits save as they are made, so review can be paused, wandered and resumed without losing work. It is a statement about *attention*, not about quality: a reviewed leg may still be marked **not used**. Distinct from **promotion**, which is one deliberate session-level decision taken later against the boat's existing polar table. An unreviewed leg carries only draft spans, so it contributes nothing. |
| **No sail** | A span carrying no sail (null `sailId`): nothing is claimed to have been flying over that time, so it feeds no polar point. The normal state of a hoist, a drop, and a cut-out bad patch — being short is what a trim *is*, not a fault. Distinct from a sailed leg being **not used**, which is a leg-level judgement that leaves every span's sail intact. |
| **Not used** (of a sailed leg) | The sailor's judgement that a whole sailed leg should not feed promotion, made beside its point count where the consequence is visible. It does **not** clear the leg's sail attributions — going back and marking it used again finds every block as it was. Contrast **no sail**, which is a span carrying nothing. |
| **Divider** | The boundary between two adjacent spans in a sailed leg. Trimming a head or tail, cutting a bad patch from the middle, and recording a sail change are all one act: moving a divider. Deleting one merges the blocks it separated, which is the universal undo. |
| **Promotion** | Turning a capture session's reviewed spans into polar points — one deliberate decision for the whole session, taken against a comparison with the boat's existing table, never a side effect of finishing review. Only **reviewed** legs take part. Promoting a session again replaces that session's points rather than adding to them. |
